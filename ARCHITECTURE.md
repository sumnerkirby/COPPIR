...
Leaflet.js with CartoDB dark tiles. The map is initialized to a US-centered view. Pins are `L.marker` instances with custom `L.divIcon` HTML that encodes security status (fill color) and operational status (border color and style). The icon HTML is constructed from validated server data only; all user-supplied strings are passed through `escHtml()` before insertion.
