...
Leaflet.js with Esri Dark Gray Canvas tiles, added as two layers: a base layer and a labels overlay. Esri orders its tile path `{z}/{y}/{x}` rather than the more common `{z}/{x}/{y}`. The service caches only to zoom 16 and returns a light placeholder tile above that, so both layers set `maxNativeZoom: 16` with `maxZoom: 19`; Leaflet upscales the zoom-16 tiles for closer views. The previous CartoDB basemap was dropped because CARTO moved it behind an API key and began serving watermarked tiles to unauthenticated clients with an HTTP 200, which no client-side error path could detect. The map is initialized to a US-centered view. Pins are `L.marker` instances with custom `L.divIcon` HTML that encodes security status (fill color) and operational status (border color and style). The icon HTML is constructed from validated server data only; all user-supplied strings are passed through `escHtml()` before insertion.
